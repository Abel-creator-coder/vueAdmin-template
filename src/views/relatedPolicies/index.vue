  	...
  	<div class="tabWrap">
  		<div style="display: inline-block;background-color: #fff;">
  			...
  			<span @click="goToItem('orgDynamics')">组织动态</span>
				<span  class="tabsel">相关政策</span>
  		</div>
  	</div>
  	<div class="brumbWrap">
  		<span>您的位置：社会影响>相关政策</span>
  	</div>
  	<div class="content1Wrap" >
      <div v-for="policy, key in policies"  @click="goDetail(policy.id)" :class="{'listWrap':true,'listactive':key==listactiveFlag}" @mouseenter="listin(key)" @mouseleave="listout(key)">
        <div class="listContentWrap">
          <p class="title">{{ policy.title }}</p>
					<p class="createdTime">{{ policy.createdAt | timeFilterFun }}</p>
        </div>
				<hr class="content1Hr">
      </div>
    </div>
...
import {getPolicyList} from '@/api/effort.js'
...
export default {
  ...
  methods: {
		init(){
      getPolicyList(this.pageNum,this.pageSize).then(reponse => {
...
.content1Wrap .listContentWrap .createdTime{
	float:right;
  font-size: 15px;
  margin-bottom:20px;
  /* margin-left:20px; */
...
.content1Hr {
	border:0.1px solid rgb(191, 192, 193);
	margin:0;
}
</style>
