  ...
  <div>
  	...
  	<div class="tabWrap">
  		<div style="display: inline-block;background-color: #fff;">
  			...
  			<span  class="tabsel">组织动态</span>
				<span @click="goToItem('relatedPolicies')">相关政策</span>
  		</div>
  	</div>
  	<div class="brumbWrap">
  		<span>您的位置：社会影响>组织动态</span>
  	</div>
  	<div class="content1Wrap" >
      <div v-for="org, key in orgs" @click="goDetail(org.id)" :class="{'listWrap':true,'listactive':key==listactiveFlag}" @mouseenter="listin(key)" @mouseleave="listout(key)">
        <img v-bind:src="org.image">
        <div class="listContentWrap">
          <p class="title">{{ org.title }}</p>
          <p class="content">{{ org.content }}</p>
        </div>
      </div>
...
import {getOrgList} from '@/api/effort.js'
...
export default {
  ...
  methods: {
		init(){
      getOrgList(this.pageNum,this.pageSize).then(reponse => {
